// Smoke test for GoogleAgent.
//
// Runs ONE debater turn against the real Gemini API. Verifies:
...
//   - Abort works
//
// Usage: cd server && node scripts/test-google.js
//   (Optionally: MODEL=gemini-2.5-flash node scripts/test-google.js)
//
...
async function testNormalCompletion() {
  ...
  if (!process.env.GOOGLE_API_KEY) {
    throw new Error('GOOGLE_API_KEY is not set in /server/.env');
  }

  const agent = new GoogleAgent(baseConfig);
  const events = [];
