// Smoke test for OpenAIAgent.
//
// Runs ONE debater turn against the real OpenAI API. Verifies:
//   - Stream emits text_delta, possibly tool_call_start/end, and exactly one turn_complete
//   - turn_complete has non-empty content
//   - Token counts populated
//   - Abort works
//
// Usage: cd server && node scripts/test-openai.js
//   (Optionally: MODEL=gpt-5-mini node scripts/test-openai.js)
//
// Cost: ~$0.05-$0.20 per full run.

import 'dotenv/config';
import { OpenAIAgent } from '../src/agents/OpenAIAgent.js';
...
async function testNormalCompletion() {
  ...
  const agent = new OpenAIAgent(baseConfig);
  const events = [];
